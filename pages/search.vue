<template>
  <div
      ...
      <div class="max-w-[1240px] mx-auto px-4 py-4">
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <div class="flex gap-3 transition-all duration-300">
            <el-button v-for="(item, index) in categories" :key="index"
              :class="[
                ...
                category === item.value ? 'shadow-lg !text-white scale-105' : 'hover:scale-105 hover:bg-white/50 dark:hover:bg-gray-600/50',
                category === item.value ? '!bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600' : ''
              ]" 
          ...
          <div v-if="category === 'clouddrive' && loadingProgress.isLoading"
            ...
            <span class="text-sm text-purple-600 dark:text-purple-400 font-medium whitespace-nowrap flex items-center gap-2">
    ...
    <div class="mx-auto overflow-y-auto px-4 scroll-smooth">
      <div class="max-w-[1240px] mx-auto">
        ...
        <transition name="fade" mode="out-in">
          <div v-if="category === 'onlineVod'" class="p-2">
            <div class="space-y-4">
              <!-- Loading Skeletons -->
              <template v-if="vodData.length === 0 && Array.from(loadingStatus.values()).some(status => status)">
                <!-- Header Skeleton -->
              ...
              <template v-else>
                <vod-list 
                  ...
                  :class="{'opacity-0': Array.from(loadingStatus.values()).some(status => status)}"
                ></vod-list>
              </template>
            </div>
          </div>
        ...
        <transition name="fade" mode="out-in">
          <div v-if="category === 'soupian'" class="h-full w-full py-4">
            <div class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-lg 
                      shadow-sm hover:shadow-lg hover:bg-white/70 dark:hover:bg-gray-800/70 p-5
                      ring-1 ring-black/5 dark:ring-white/5 transition-all duration-300">
              <div class="relative w-full h-[calc(100vh-240px)]">
                <div v-if="!iframeLoaded" class="absolute inset-0 flex items-center justify-center bg-white/90 dark:bg-gray-800/90 rounded-lg
                            backdrop-blur-sm">
      ...
      <div v-if="!skeletonLoading && !Array.from(loadingStatus.values()).some(status => status) && 
        ...
        <div class="p-8 rounded-full bg-gradient-to-br from-purple-100/50 to-blue-100/50 dark:from-purple-900/20 dark:to-blue-900/20 mb-6 
                    shadow-lg backdrop-blur-sm group hover:scale-105 transition-all duration-300">
          <i class="fas fa-search text-5xl text-purple-500/70 dark:text-purple-400/70 group-hover:rotate-12 transition-transform duration-300"></i>
        </div>
        <h3 class="text-xl font-semibold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">未找到相关结果</h3>
...
class SmartCache {
  ...
  getDynamicTTL(key, value) {
    ...
    return Math.floor(baseTime * sizeMultiplier * hitRateMultiplier * frequencyMultiplier);
  }
